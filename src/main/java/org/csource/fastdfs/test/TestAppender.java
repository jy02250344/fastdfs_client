...
public class TestAppender {
	...
	public static void main(String args[]) {
		...
		try {
			...
			results = client.upload_appender_file(local_filename, null,
					meta_list);
			if (results != null) {
				String file_id;
				int ts;
			...
			if (results != null) {
				...
				startTime = System.currentTimeMillis();
				errno = client.append_file(group_name, appender_filename,
						f.length(), new UploadLocalFileSender(local_filename));
				System.out.println("append_file time used: "
						+ (System.currentTimeMillis() - startTime) + " ms");
				if (errno == 0) {
					System.err.println(client.get_file_info(group_name,
				...
				startTime = System.currentTimeMillis();
				errno = client.modify_file(group_name, appender_filename, 0,
						f.length(), new UploadLocalFileSender(local_filename));
				...
				if (errno == 0) {
					...
					System.err
							.println("truncate file fail, error no: " + errno);
				}
			} else {
