...
public class TestAppender1 {
	...
	public static void main(String args[]) {
		...
		try {
			...
			appender_file_id = client.upload_appender_file1(file_buff, "txt",
					meta_list);
			...
			if (appender_file_id == null) {
				...
				servers = tracker.getFetchStorages1(trackerServer,
						appender_file_id);
				if (servers == null) {
			...
			appender_file_id = client.upload_appender_file1(local_filename,
					null, meta_list);
			if (appender_file_id != null) {
				int ts;
				String token;
				...
				if (ClientGlobal.g_anti_steal_token) {
					...
					token = ProtoCommon.getToken(appender_file_id, ts,
				...
				errno = client.download_file1(appender_file_id, 0, 0, "c:\\"
						+ appender_file_id.replaceAll("/", "_"));
				if (errno == 0) {
				...
				errno = client.download_file1(
						appender_file_id,
						0,
						0,
						new DownloadFileWriter("c:\\"
								+ appender_file_id.replaceAll("/", "-")));
				if (errno == 0) {
					System.err.println("Download file success");
				} else {
					System.err
			...
			storageServer = tracker.getFetchStorage1(trackerServer,
					appender_file_id);
